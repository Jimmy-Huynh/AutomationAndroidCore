...
/**
 * Created by huutam.huynh on 2/27/18.
 */
...
public class CountDownAnimation {
    ...
    private boolean isAllowCountdown;
    private Handler mHandler = new Handler();

    private final Runnable mCountDown = new Runnable() {
        public void run() {
            if (mCurrentCount > 0) {
                mTextView.setText(statusText + (isAllowCountdown ? mCurrentCount : "") + "");
    ...
    public CountDownAnimation(TextView textView, int startCount, String statusText, boolean isAllowCountdown) {
        this.mTextView = textView;
        this.mStartCount = startCount;
        this.statusText = statusText;
        this.isAllowCountdown = isAllowCountdown;
    ...
    public void start() {
        ...
        mTextView.setText(statusText + (isAllowCountdown ? mStartCount : "") + "");
        ...
        for (int i = 1; i <= mStartCount; i++) {
            mHandler.postDelayed(mCountDown, i * mAnimation.getDuration());
        }
    }
